...
Despite the improvements made by the JavaScript community, there are several issues that make developing software using JavaScript hard. Working in JavaScript is inconvenient because of its module system (not available before ES6~\cite{}), its weak-typing, the verbose function syntax\footnote{its support for closures is commonly noted as being one of JavaScript\rq{s} redeeming features.}, the late binding\footnote{Early binding allows for static verification of the existence of method-signature pairs}, the lack of static types, the behavior of \emph{this}, the equality/automatic conversion to boolean and the asynchronous programming heavily based on callbacks.

There are interesting projects such as Dart~\cite{}, Typescript~\cite{}, Elm~\cite{} and Scala.js~\cite{} that address some of the issues described above, allowing us to write safe code, but they do not tackle the problem of writing highly concurrent and asynchronous code.
