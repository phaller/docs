...
Since it introduction in 1995~\cite{} there has been an increasing interest on JavaScript. Originally, this programming language was developed by Netscape Inc. to provide a lightweight interpreted language to captivate non-professional programmers. Later, thanks to a standardisation process started in 1996 with the first release~\cite{} and, that continues nowadays with the sixth version released in June 2015~\cite{}, Javascript has become one of the most popular programming languages on the Web.

It is interesting to analyze the evolution of the language. Before the introduction of Ajax~\cite{} Javascript was principally used to do scripting in the Browser. In 2009 CommonJS~\cite{} and Node.js~\cite{web:nodejs} proposed an ecosystem for running Javascript outside the browser. With the rise of single-page web applications~\cite{} and source-to-source compilers~\cite{}, Javascript is now being used to power a large variety of projects. 

Despite the advances made by the javascript community, there are several issues that make developing software using Javascript harder. Working in Javascript is inconvenient because of its module system (only in ES6~\cite{}), its weak-typing, the verbose function syntax\footnote{Its support for closures is commonly noted as being one of JavaScript\rq{s} redeeming features.}, the late binding\footnote{Early binding allows for static verification of the existence of method-signature pairs}, the lack of static types, the \emph{this} behaviour, the equality/automatic conversion to boolean and the asynchronous programming heavily based on callbacks.

There are interesting projects such as Dart~\cite{}, Typescript~\cite{}, Elm~\cite{} and Scala.js~\cite{} that address some of the above described issues allowing us to write safe code, but they do not tackle the problem of writing highly concurrent and asynchronous code.

In this paper we address this problem by presenting Akka.js, an implementation of an actor system in Scala.js. To goal of this project is to expose a model and an API similar to Akka, so that Scala developers can reuse transparently knowledge between the client-code (\emph{Browser}) and the server-code.

{\color{blue} 
Now we need to describe:
\begin{enumerate}
\item why an actor system
\item why scala.js? maybe not?
\item an enumeration of the feature implemented
\item a remained to the other sections of the paper
\end{enumerate}
}


{\color{red}
TO REMOVE (can we reuse some of this?)
...
Jetlang. (See \url{https://code.google.com/p/retlang/} and \url{https://github.com/jetlang})

Fantom. (See \url{http://www.fandev.org/}) It supports actors, and the
language compiles to several platforms including the JVM and JavaScript.
